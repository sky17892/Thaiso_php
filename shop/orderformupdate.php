...
//echo "order_address_image : ".$order_address_image . "</br>";
//echo "order_address_image : ".$_FILES['order_address_image']['name'] . "</br>";
//if ($_FILES['order_address_image']['name']) {
	@mkdir(G5_DATA_PATH."/order/".$od_id, G5_DIR_PERMISSION);
	@chmod(G5_DATA_PATH."/order/".$od_id, G5_DIR_PERMISSION);
	//data/cs_files/
	$sql_address = "";

	if ($_FILES['order_address_image']['name']) {
//echo "2order_address_image : ".$_FILES['order_address_image']['name'] . "</br>";
		$tmp_file  = $_FILES['order_address_image']['tmp_name'];
		$filename  = $_FILES['order_address_image']['name'];
...
//echo "3order_address_image : ".$_FILES['order_address_image']['name'] . "</br>";
		// 첨부파일 첨부시 첨부파일명에 공백이 포함되어 있으면 일부 PC에서 보이지 않거나 다운로드 되지 않는 현상이 있습니다. (길상여의 님 090925)
		$filename = md5(sha1($_SERVER['REMOTE_ADDR'])).'_'.substr($shuffle,0,8).'_'.replace_filename($filename);
		$dest_path = '/order/';
		$dest_file = G5_DATA_PATH.'/order/'.$od_id.'/'.$filename;
		echo "3333333".G5_DATA_PATH.'/order/'.$od_id.'/'.$filename;
		$file_path = '/data/order/'.$od_id.'/'.$filename;

		$error_code = move_uploaded_file($tmp_file, $dest_file) or die($_FILES['order_address_image']['error']);
		$sql_address .= ", order_address_image = '$filename'";
