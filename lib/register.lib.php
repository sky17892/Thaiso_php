...
function exist_mb_code($reg_mb_code)
{
    global $g5;

    $reg_mb_code = trim($reg_mb_code);
    if ($reg_mb_code == "") return "";

    //$sql = " select count(*) as cnt from `{$g5['member_table']}` where mb_code = '$reg_mb_code' ";
    $sql = " select count(*) as cnt from `{$g5['member_table']}` where mb_id = '$mb_id' ";
    $row = sql_fetch($sql);
    if ($row['cnt'])
        return "이미 사용중인 코드 입니다.";
    else
        return "";
}
