...
if ($w == '' || $w == 'u') {
	@mkdir(G5_DATA_PATH."/address", G5_DIR_PERMISSION);
	@chmod(G5_DATA_PATH."/address", G5_DIR_PERMISSION);
	//data/cs_files/
	$sql_address = "";
	// 파일삭제
	if ($del_mb_address_image) {
		$address_image = G5_DATA_PATH."/address/".clean_relative_paths($member['mb_address_image']);
		@unlink($address_image);
	}

	if ($_FILES['mb_address_image']['name']) {
		$tmp_file  = $_FILES['mb_address_image']['tmp_name'];
		$filename  = $_FILES['mb_address_image']['name'];

		$ofilename  = get_safe_filename($filename);
		...
		$dest_path = '/address/';
		$dest_file = G5_DATA_PATH.'/address/'.$filename;
		$file_path = '/data/address/'.$filename;

		$error_code = move_uploaded_file($tmp_file, $dest_file) or die($_FILES['mb_address_image']['error']);
		$sql_address .= ", mb_address_image = '$filename'";
    ...
    if ($w=='') {
        ...
        if ($mb['mb_id']) {
            $mb_recomm_code = $mb_recommend;
            $mb_recommend = $mb['mb_id'];
        }
        if ($mb_recommend && (strtolower($mb['mb_code']) == strtolower($mb_recommend))) {
			//alert ("-".strtolower($mb['mb_code'])." == ".strtolower($mb_recommend));
