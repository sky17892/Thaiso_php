...
<div class="shop-cart">
            ...
            <table class="table">
                <thead>
                    <tr>
                        ...
                        <th class="tbd-both">ชื่อสินค้า</th>
                        <th class="width-100px">ปริมาณรวม</th>
                        <th class="width-100px tbd-both">ราคาขาย</th>
                        <th class="width-100px">พอยท์</th>
                        <th class="width-100px tbd-both">ค่าจัดส่ง</th>
                        <th class="width-100px">ผลรวม</th>
                    </tr>
                </thead>
                <tbody>
                    <?php if ($count > 0) { ?>
                        ...
                        <td class="tbd-both">
                            <div class="td-item-desc">
                                ...
                                <div class="td-item-name">
                                    ...
                                    <div class="m-t-15"><button type="button" class="btn-e btn-e-dark mod_options" data-bs-toggle="modal" data-bs-target="#modal_mod_option">ปรับเปลี่ยนการเลือก</button></div>
                                    <?php } ?>
                                </div>
                            </div>
                        </td>
                    ...
                    <?php } ?>
                    <?php } else { ?>
                    <tr><td colspan="8" class="text-center"><span class="text-gray"><i class="fas fa-exclamation-circle"></i> ไม่มีสินค้าในตะกร้า</span></td></tr>
                    <?php } ?>
                </tbody>
            </table>
        </div>
    ...
    <?php if ($count > 0) { ?>
        ...
        <li class="shop-cart-li">
            ...
            <div class="li-item-wrap">
                ...
                <div class="li-mod" >
                    <button type="button" class="btn-e btn-e-dark mod_options" data-bs-toggle="modal" data-bs-target="#modal_mod_option">ปรับเปลี่ยนการเลือก</button>
                </div>
            </div>

            <div class="li-prqty">
                <span class="li-prqty-sp prqty-price"><span>ราคาขาย </span><?php echo number_format($list[$i]['ct_price']); ?></span>
                <span class="li-prqty-sp prqty-qty"><span>수량 </span><?php echo number_format($list[$i]['sum_qty']); ?></span>
                <span class="li-prqty-sp prqty-sc"><span>ค่าจัดส่ง </span><?php echo $list[$i]['ct_send_cost']; ?></span>
                <span class="li-prqty-sp total-point"><span>적립พอยท์ </span><strong><?php echo number_format($list[$i]['point']); ?></strong></span>
            </div>
            <div class="total-price total-span"><span>ผลรวม </span><strong id="sell_price_<?php echo $i; ?>"><?php echo number_format($list[$i]['sell_price']); ?></strong></div>
        </li>
        <?php } ?>
    <?php } else { ?>
        <li class="text-center"><span class="text-gray"><i class="fas fa-exclamation-circle"></i>ไม่มีสินค้าในตะกร้า</span></li>
    <?php } ?>
    </ul>
    ...
    <?php if ($count > 0) { ?>
    <div class="m-b-30">
        <button type="button" onclick="return form_check('seldelete');" class="btn-e btn-e-dark">เลือกลบ</button>
        <button type="button" onclick="return form_check('alldelete');" class="btn-e btn-e-crimson">ลบทั้งหมด</button>
    </div>
    <?php } ?>
    ...
    <div class="shop-cart-total">
        ...
        <div class="cart-total-box">
            <span>แนะนำ</span>
            <strong class="cart-total-price"><?php echo number_format($tot_price); ?></strong> ₩
        </div>
    </div>
    <?php } // if G5_IS_MOBILE END ?>
    ...
    <div class="cart-act-btn">
        <?php if ($i == 0) { ?>
        <a href="<?php echo G5_SHOP_URL; ?>/" class="btn-e btn-e-brd btn-e-xl btn-e-dark">ช้อปปิ้งต่อ</a>
        <?php } else { ?>
        <input type="hidden" name="url" value="<?php echo G5_SHOP_URL; ?>/orderform.php">
        <input type="hidden" name="records" value="<?php echo $i; ?>">
        <input type="hidden" name="act" value="">
        <a href="<?php echo shop_category_url($continue_ca_id); ?>" class="btn-e btn-e-brd btn-e-xl btn-e-dark">ช้อปปิ้งต่อ</a>
        <button type="button" onclick="return form_check('buy');" class="btn-e btn-e-xl btn-e-navy"><i class="fas fa-credit-card m-r-5"></i>สั่งซื้อ</button>
