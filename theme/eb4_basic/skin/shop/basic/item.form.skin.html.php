        ...
        <div class="col-md-5 md-m-b-30">
            ...
            <div class="shop-product-img">
                <div class="product-img-big">
					...
					<img src="/img/zoo_ico.png" <?php if (!G5_IS_MOBILE) { ?> data-bs-toggle="modal" data-bs-target=".shop-img-modal"<?php } ?> style="position: absolute;left: 88%;    bottom: 1px;    width: 40px;    opacity: 0.4;    background: #0000007a;   border-radius: 5px;">
            ...
            <div class="shop-product-prev-next">
                ...
                <a href="<?php echo $prev_href; ?>" class="product-prev">
                    <i class="fas fa-chevron-left" aria-hidden="true"></i> สินค้าก่อนหน้า <span class="sound_only"> <?php echo $prev_title; ?></span>
                </a>
                <a href="<?php echo $next_href; ?>" class="product-next">
                   สินค้าถัดไป <span class="sound_only"> <?php echo $next_title; ?></span> <i class="fas fa-chevron-right" aria-hidden="true"></i>
                </a>
                <?php } else { ?>
        ...
        <div class="col-md-7">
            ...
            <div class="sit-ov-height">
                <div id="sit_ov" class="shop-product-form 2017_renewal_itemform">
                    ...
                    <div class="buy-btn-wr"><button type="button" class="buy-op-btn">ซื้อสินค้า <i class="fas fa-credit-card"></i></button></div>
                    <div class="scroll-no">
                        ...
                        <div class="shop-description-box table-list-eb">
                            <table class="table">
                                <tbody>
                                    <?php if (!$it['it_use']) { // ราคาขาย능이 아닐 경우 ?>
                                    <tr>
                                    ...
                                    */
                                    ...
                                    <?php if ($config['cf_use_point']) { // พอยท์ 사용한다면 ?>
                                    <tr>
                                        <th scope="row">พอยท์</th>
                                        <td>
                                            <?php
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <select name="ct_send_cost" id="ct_send_cost">
                                                <option value="0">ชำระเงินเมื่อสั่งซื้อ</option>
                    ...
                    <div class="scroll-show">
                        ...
                        <?php if($is_soldout) { ?>
                        <p id="sit_ov_soldout">ไม่สามารถซื้อได้เนื่องจากสินค้าขาดสต็อก</p>
                        <?php } ?>

                        <div id="sit_ov_btn">
                            <?php if ($is_orderable) { ?>
                            <button type="submit" onclick="document.pressed=this.value;" value="ซื้อสินค้า" id="sit_btn_buy"><i class="fas fa-credit-card" aria-hidden="true"></i> ซื้อสินค้า</button>
                            <button type="submit" onclick="document.pressed=this.value;" value="ตะกร้าสินค้า" id="sit_btn_cart"><i class="fas fa-cart-arrow-down" aria-hidden="true"></i> ตะกร้าสินค้า</button>
                            <?php } ?>
                            <?php if(!$is_orderable && $it['it_soldout'] && $it['it_stock_sms']) { ?>
                            <a href="javascript:popup_stocksms('<?php echo $it['it_id']; ?>');" id="sit_btn_alm"><i class="far fa-bell" aria-hidden="true"></i> การแจ้งเตือนการกลับมาในสต็อก</a>
                            <?php } ?>
                        ...
                        function popup_item_recommend(it_id) {
                            if (!g5_is_member)
                            {
                                if (confirm("เฉพาะสมาชิกเท่านั้นที่สามารถแนะนำได้"))
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button data-bs-dismiss="modal" aria-label="Close" class="btn-e btn-e-lg btn-e-dark" type="button"><i class="fas fa-times"></i> ปิด</button>
            </div>
        </div>
    </div>
</div>
...
function fsubmit_check(f)
{
    ...
    if (document.getElementById("it_price").value < 0) {
        alert("กรุณาติดต่อทางโทรศัพท์ ขอบคุณค่ะ/ครับ");
        return false;
    }

    if($(".sit_opt_list").size() < 1) {
        alert("กรุณาเลือกตัวเลือกเสริมของสินค้า");
        return false;
    }
    ...
    $("input[name^=ct_qty]").each(function(index) {
        ...
        if(parseInt(val.replace(/[^0-9]/g, "")) < 1) {
            alert("กรุณาใส่อย่างน้อย 1 จำนวน");
            result = false;
            return false;
        }
...
// 바로구매, ตะกร้าสินค้า 폼 전송
function fitem_submit(f)
{
    f.action = "<?php echo $action_url; ?>";
    f.target = "";

    if (document.pressed == "ตะกร้าสินค้า") {
        f.sw_direct.value = 0;
    } else { // 바로구매
        f.sw_direct.value = 1;
    }
