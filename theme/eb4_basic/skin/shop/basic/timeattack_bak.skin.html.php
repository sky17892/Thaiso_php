...
<?php echo eb_slider('1730822706'); ?>
    ...
    <?php for ($i=0; $row=sql_fetch_array($result); $i++) { 
				$row['href'] = get_pretty_url('shop', $row['it_id']);
				$row['it_image'] = get_it_image($row['it_id'], 300, 300);
    ...
    <div class="item-list-wrap">
        <div class="item-list">
            <div style="float:left"><?php if ($row['href']) { ?><a href="<?php echo $row['href']; ?>"><?php } ?><?php echo $row['it_image']; ?><?php if ($row['href']) { ?></a><?php } ?></div>
            ...
            <div class="product-description">
                <div class="product-description-in">
                    <h5 class="product-name">
                        ...
                        <?php } ?>
                        <?php if ($row['it_name']) { echo stripslashes($row['it_name']); } ?>
                        <?php if ($row['href']) { ?>
                        </a>
                        <?php } ?>
                    </h5>
                    <?php //if ($this->view_it_basic) { ?>
                    <div class="product-info"><?php echo stripslashes($row['it_basic']); ?></div>
                    <?php //} ?>                        <?php 
if($row['it_cust_price'] > 0 && $row['it_price'] > 0 ) {
				
	$row['dc_ratio'] = 100 - intval(( $row['it_price'] /  $row['it_cust_price'] ) * 100);
}
			
			if($row['dc_ratio']) { ?>
                        <div class="discount-percent"><?php echo $row['dc_ratio']; ?>%</div>
                        <?php } ?>
                    <?php //if ($this->view_it_cust_price || $this->view_it_price) { ?>
                    <div class="product-price">

                        <?php //if ($this->view_it_price) { ?><span class="title-price">₩ <?php echo number_format($row['it_price']); ?></span><br>
                        <?php //} ?>
                        <?php //if ($this->view_it_cust_price && $row['it_cust_price']) { ?><span class="title-price line-through">₩ <?php echo $row['it_cust_price']; ?></span><br>
                        <?php //} ?>
						재고량 : <?php echo number_format($row['it_stock_qty']); ?>
                    </div>
                    <?php //} ?>
                    ...
                    <div class="product-sns">
                        <ul>
                            <li><a href="javascript:void(0);" class="wish-icon" onclick="item_wish_for_list(<?php echo $row['it_id']; ?>);" title="위시리스트"><img src="<?php echo EYOOM_THEME_URL; ?>/image/social/heart-regular.svg" alt=""></a></li>
                            <li><a href="https://www.facebook.com/sharer/sharer.php?u=<?php echo $row['sns_url']; ?>&amp;p=<?php echo $row['sns_title']; ?>" target="_blank" class="facebook-icon" title="페이스북"><img src="<?php echo EYOOM_THEME_URL; ?>/image/social/facebook-f.svg" alt=""></a></li>
                            <li><a href="https://twitter.com/share?url=<?php echo $row['sns_url']; ?>&amp;text=<?php echo $row['sns_title']; ?>" target="_blank" class="twitter-icon" title="엑스(트위터)"><img src="<?php echo EYOOM_THEME_URL; ?>/image/social/x-twitter.svg" alt=""></a></li>
                        </ul>
                    </div>
                    <?php //} ?>
                </div>
            ...
            <div class="adm-edit-btn btn-edit-mode" style="bottom:0">
                <div class="btn-group">
                    <a href="<?php echo G5_ADMIN_URL; ?>/?dir=shop&pid=itemform&w=u&it_id=<?php echo $row['it_id']; ?>&wmode=1" onclick="eb_admset_modal(this.href); return false;" class="ae-btn-l ae-item-btn"><i class="far fa-edit"></i> 개별상품 설정</a>
                    <a href="<?php echo G5_ADMIN_URL; ?>/?dir=shop&pid=itemform&w=u&it_id=<?php echo $row['it_id']; ?>" target="_blank" class="ae-btn-r" title="새창 열기">
