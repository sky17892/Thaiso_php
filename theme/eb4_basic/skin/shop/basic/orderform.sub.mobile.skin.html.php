                ...
                <div class="order-table">
                    <table>
                        <tbody>
                            ...
                            <tr>
                                <th scope="row"><label for="od_b_hp">โทรศัพท์มือถือ</label></th>
                                <td>
                                    <label for="od_b_hp" class="label hidden-lg hidden-md">โทรศัพท์มือถือ</label>
                                    <label class="input width-200px">
                            ...
                            <tr>
                                ...
                                <td id="sod_frm_addr">
                                    <label class="label hidden-lg hidden-md">ที่อยู่</label>
                                    <div class="clearfix"></div>
                                    <label for="od_b_zip" class="sound_only">รหัสไปรษณีย์<strong class="sound_only"> 필수</strong></label>
                                    <label class="input width-150px m-r-5 float-start">
                                        <i class="icon-append fas fa-question-circle"></i>
                                        <input type="text" name="od_b_zip" id="od_b_zip" required class="required" size="8" maxlength="6" placeholder="รหัสไปรษณีย์">
                                        <b class="tooltip tooltip-top-right">รหัสไปรษณีย์ (ค้นหาโดยคลิกปุ่มค้นหาที่อยู่)</b>
                                    </label>
                                    <button type="button" class="btn-e btn-e-lg btn-e-navy" onclick="win_zip('forderform', 'od_b_zip', 'od_b_addr1', 'od_b_addr2', 'od_b_addr3', 'od_b_addr_jibeon');">ค้นหา</button>
                                    <div class="clearfix m-b-10"></div>
                                    <label for="od_b_addr1" class="sound_only">ที่อยู่พื้นฐาน<strong class="sound_only"> 필수</strong></label>
                                    <label class="input ">
                                        <input type="text" name="od_b_addr1" id="od_b_addr1" size="60" placeholder="ที่อยู่พื้นฐานู่">
                                    </label>
                                    <label for="od_b_addr2" class="sound_only">ที่อยู่โดยละเอียดู่</label>
                                    <label class="input">
                                        <input type="text" name="od_b_addr2" id="od_b_addr2" size="60" placeholder="ที่อยู่โดยละเอียดู่">
                                    </label>
                                    <label for="od_b_addr3" class="sound_only">รายการอ้างอิง</label>
                                    <label class="input">
                                        <input type="text" name="od_b_addr3" id="od_b_addr3" readonly="readonly" size="60" placeholder="รายการอ้างอิง">
                                    </label>
                                    <input type="hidden" name="od_b_addr_jibeon" value="">
<style type="text/css">
	.bank_icon {display:none}
	.PAYNOW {display:none}
	.shop-order-form .payment-select-wrap .payment-select-box {width:100% !important}
</style>

            <div class="row">
                <section class="col-lg-12">
                    <label for="order_address_image" class="label">แนบรูปภาพที่อยู่xxx</label>
                    <div class="input m-b-10">
                        <!--input type="file" class="form-control" id="order_address_image" name="order_address_image" value="เลือกไฟล์"-->
						<input 
                type="file" 
                class="form-control" 
                id="order_address_image" 
                name="order_address_image" 
                accept="image/*" 
                capture="environment" value="เลือกไฟล์"
            >
                    </div>
                    <div class="clearfix"></div>
                    <?php 
$mb_address_path = G5_DATA_PATH.'/address/'.$member['mb_address_image'];
$mb_address_url = G5_DATA_URL.'/address/'.$member['mb_address_image'];
					
					if ($w == 'u' && file_exists($mb_address_path)) {  ?>
                    <img src="<?php echo $mb_address_url ?>" alt="주소이미지첨부" class="float-start m-r-10">
                    <label class="checkbox float-start p-l-20"><input type="checkbox" name="del_mb_address_image" value="1" id="del_mb_address_image"><i></i>삭제</label>
                    <div class="clearfix"></div>
                    <?php }  ?>
                </section>
            </div>
                            ...
                            <tr>
                                <th scope="row"><label for="od_memo">บันทึก</label></th>
                                <td>
                                    <label for="od_memo" class="label hidden-lg hidden-md">บันทึก</label>
        ...
        <div class="order-payment-area">
            ...
            <div class="order-payment-info">
                ...
                <div class="payment-info-box">
                    <span>ค่าจัดส่ง할인</span>
                    <strong id="sc_cp_price">0</strong>₩
                ...
                <div class="payment-info-box">
                    ...
                    <strong id="od_send_cost2">0</strong>₩
                    <div class="f-s-13r text-gray m-t-10"><b class="text-crimson">*</b> ค่าจัดส่งเช่นค่าธรรมเนียมการนำร่องจะถูกเพิ่มขึ้นอยู่กับภูมิภาค</div>
                </div>
                <div class="m-b-20"></div>
                <div id="od_tot_price" class="payment-info-box border-color-red">
                    <span class="text-black">ยอดสั่งซื้อทั้งหมด</span>
                    <strong class="print_price"><?php echo number_format($tot_price); ?></strong>₩
                </div>

                <div class="payment-select-wrap">
                    <h5 class="payment-select-title"><strong>วิธีการชำระเงิน</strong></h5>
                    <?php if (!$default['de_card_point']) { ?>
                    <p class="text-gray f-s-13r m-b-15"><b class="text-crimson">*</b> หากคุณชำระเงินด้วยวิธีการชำระเงินอื่นที่ไม่ใช่การโอนเงินผ่านธนาคาร คะแนนจะไม่สะสม</p>
                    <?php } ?>
                    ...
                    <input type="radio" id="od_settle_bank" name="od_settle_case" value="무통장" <?php echo $checked; ?>><label for="od_settle_bank" class="payment-select-box bank_icon">โอนเงินผ่านธนาคาร</label>
                    <?php $checked = ''; } ?>

                    <?php if($default['de_vbank_use']) { $multi_settle++; // 가상계좌 사용 ?>
                    <input type="radio" id="od_settle_vbank" name="od_settle_case" value="가상계좌" <?php echo $checked; ?>><label for="od_settle_vbank" class="payment-select-box vbank_icon"><?php echo $escrow_title; ?>บัญชีเสมือน</label>
                    <?php $checked = ''; } ?>
                    ...
                    <?php if ($temp_point) { // 회원이면서 พอยท์사용이면 ?>
                    <div class="payment-point-use-box">
                        <div class="payment-point-use">
                            <label for="od_temp_point" class="float-start">ใช้พอยท์<?php echo $point_unit; ?>พอยท์)</label>
                            <div class="float-end width-120px">
                                <input type="hidden" name="max_temp_point" value="<?php echo $temp_point; ?>">
                                <label class="input">
                                <i class="icon-append font-style-normal">พอยท์</i>
                                <input type="text" name="od_temp_point" value="0" id="od_temp_point" size="7">
                                </label>
                            </div>
                        </div>
                        <div class="payment-point-use">
                            <strong class="float-start">พอยท์ที่มีอยู่</strong><span class="float-end"><?php echo display_point($member['mb_point']); ?></span>
                            <div class="clearfix"></div>
                            <strong class="float-start">พอยท์ที่สามารถใช้ได้สูงสุด</strong><strong class="float-end text-black"><?php echo display_point($temp_point); ?></strong>
                        </div>
                    </div>
                    <?php $multi_settle++; } ?>
...
$(function() {
    ...
    $(document).on("click", ".cp_apply", function() {
        ...
        if(cp_dup) {
            ...
            if(!confirm(subj+ "쿠폰은\n"+it_name+"에 사용되었습니다.\n"+it_name+"의 쿠폰을 ยกเลิก한 후 적용하시겠습니까?")) {
                return false;
            } else {
        ...
        if(!$cp_row_el.find(".cp_cancel").length)
            $cp_btn_el.after("<button type=\"button\" class=\"btn-e btn-e-gray cp_cancel\">ยกเลิก</button>");
    });
    ...
    $(document).on("click", ".od_cp_apply", function() {
        ...
        if(!$("#od_coupon_cancel").length)
            $("#od_coupon_btn").after("<button type=\"button\" id=\"od_coupon_cancel\" class=\"btn-e btn-e-gray cp_cancel\">ยกเลิก</button>");
    });
    ...
    $(document).on("click", ".sc_cp_apply", function() {
        ...
        if(!$("#sc_coupon_cancel").length)
            $("#sc_coupon_btn").after("<button type=\"button\" id=\"sc_coupon_cancel\" class=\"btn-e btn-e-gray cp_cancel\">ยกเลิก</button>");
    });
...
function forderform_check(f)
{
    ...
    check_field(f.od_name, "คนสั่งของ ชื่อ을 입력하십시오.");
    ...
    check_field(f.od_tel, "คนสั่งของ หมายเลขโทรศัพท์를 입력하십시오.");
    //check_field(f.od_addr1, "ที่อยู่검색을 이용하여 คนสั่งของ ที่อยู่를 입력하십시오.");
    //check_field(f.od_addr2, " คนสั่งของ의 상세ที่อยู่를 입력하십시오.");
    ...
    check_field(f.od_b_name, "받으시는 분 ชื่อ을 입력하십시오.");
    check_field(f.od_b_tel, "받으시는 분 หมายเลขโทรศัพท์를 입력하십시오.");
   // check_field(f.od_b_addr1, "ที่อยู่검색을 이용하여 받으시는 분 ที่อยู่를 입력하십시오.");
    //check_field(f.od_b_addr2, "받으시는 분의 상세ที่อยู่를 입력하십시오.");
   ///check_field(f.od_b_zip, "");
    ...
    // ค่าจัดส่ง를 받지 않거나 더 받는 경우 아래식에 + 또는 - 로 대입
    ...
    if (typeof(f.od_temp_point) != "undefined") {
        ...
        if (f.od_temp_point.value)
        {
            if (temp_point > od_price) {
                alert("상품 주문금액(ค่าจัดส่ง 제외) 보다 많이 พอยท์결제할 수 없습니다.");
                f.od_temp_point.select();
                return false;
            }

            if (temp_point > <?php echo (int)$member['mb_point']; ?>) {
                alert("회원님의 พอยท์보다 많이 결제할 수 없습니다.");
                f.od_temp_point.select();
                return false;
            }
            ...
            if (parseInt(parseInt(temp_point / point_unit) * point_unit) != temp_point) {
                alert("พอยท์를 "+String(point_unit)+"점 단위로 입력하세요.");
                f.od_temp_point.select();
                return false;
            }
        }

        // pg 결제 금액에서 พอยท์ 금액 차감
        ...
        switch(settle_method)
        {
            ...
            case "간편결제":
                ...
                if(nicepay_easy_pay === "nice_naverpay"){
                    ...
                    <?php 
                        // * 카드 선택 시 전액 카드로 결제, พอยท์ 선택 시 전액 พอยท์로 결제.
                        // (카드와 พอยท์를 같이 사용하는 복합결제 형태의 결제는 불가함.)
                        // - 카드: EasyPayMethod=”E020=CARD”, พอยท์: EasyPayMethod=”E020=POINT”
                    ?>
