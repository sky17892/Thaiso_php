...
$sql_search = " where (a.ts_id = '$ts_id') ";
if ($stx) {
    $sql_search .= " and ( ";
    switch ($sfl) {
        case 'mb_point':
...
$sql = " select a.it_id, b.* {$sql_common} {$sql_search} {$sql_order} limit {$from_record}, {$rows} ";
...
for ($i=0; $row=sql_fetch_array($result); $i++) {
	...
	//$list[$i]['btn_add_timesale'] = "<input type='submit' name='act_button' value='제거' class='btn-e btn-e-xs btn-e-red' onclick='document.pressed=this.value'>";
	$list[$i]['btn_add_timesale'] = "<a href='javascript:;' onClick=delete_ts('".$row['it_id']."'); class='btn-e btn-e-red btn-e-xs'>제거</a>";
