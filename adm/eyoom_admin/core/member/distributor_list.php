...
$action_url1 = G5_ADMIN_URL . '/?dir=member&amp;pid=distributor_list_update&amp;smode=1';
...
for ($i=0; $row=sql_fetch_array($result); $i++) {
    ...
    $list[$i] = $row;
    $k = $list[$i]['mb_level2'];
    $list[$i]['mb_dlevel'] = $k . '등급';
	$list[$i]['mb_point_rate'] = $config['cf_settle_point'.$k];
	/*
    switch($row['mb_level']) {
        case '5':
            $list[$i]['mb_dlevel'] = '3등급';
			$list[$i]['mb_point_rate'] = $config['cf_settle_point3'];
            break;
        case '6':
            $list[$i]['mb_dlevel'] = '2등급';
			$list[$i]['mb_point_rate'] = $config['cf_settle_point2'];
            break;
        case '7':
            $list[$i]['mb_dlevel'] = '1등급';
 			$list[$i]['mb_point_rate'] = $config['cf_settle_point1'];
           break;
        default:
            $list[$i]['mb_dlevel'] = '-';
            break;
