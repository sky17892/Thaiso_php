                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-center">
							...
							<?php } ?>
							<?php if ($config['cf_admin'] != $list[$i]['mb_id']) { ?>
								</br><a class="btn-e btn-e-xs btn-e-green" href="javascript:openModal('<?php echo $list[$i]['mb_id']; ?>','<?php echo $list[$i]['mb_name']; ?>','<?php echo $list[$i]['mb_level2']; ?>');" class="m-l-10">총판승인</a>
							<?php } ?>
                        </td>
                        <?php } else { ?>
                        <td class="text-center">
                        ...
                        <td><strong><?php echo $list[$i]['mb_code']; ?></strong>
                        ...
                        <td>
							<?php if( $list[$i]['mb_recommend']) { 
								$sp = get_member($list[$i]['mb_recommend']);
							?>
                            <a class="btn-e btn-e-xs btn-e-blue" href="<?php echo G5_ADMIN_URL; ?>/?dir=member&pid=member_form&mb_id=<?php echo $sp['mb_id'];; ?>&w=u&amp;wmode=1" onclick="eb_modal(this.href); return false;"><?php echo get_text($list[$i]['mb_recommend']); ?></a>
							<?php } ?>
                        </td>
                        <?php if (!$wmode) { ?>
                        <!--<td>
...
<div class="modal fade admin-iframe-modal" tabindex="-1" aria-hidden="true">
	...
	<input type="hidden" name="mmb_name" id="mmb_name" value="">
        <div class="modal-content">
            ...
            <br>
            <button type="button" onclick="confirmSelection()">승인처리</button>
...
<script>
	...
	if(confirm(mb_name+"("+mb_id+")님을 총판 ("+ mlevel +")등급으로 변경하시겠습니까?")) {
		$.ajax({
			type: "POST",
			url: g5_bbs_url+"/ajax.level_up.php",
			data: {
				"mb_id": mb_id,
...
function levelUp(mb_id,mb_name) {
	if(confirm(mb_name+"("+mb_id+")님을 총판 등급으로 변경하시겠습니까?")) {
		$.ajax({
			type: "POST",
			url: g5_bbs_url+"/ajax.level_up.php",
			data: {
