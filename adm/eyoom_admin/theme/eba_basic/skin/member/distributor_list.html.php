...
$fm_pid = 'member_list';
$g5_title = '총판관리';
$g5_page_path = '<li class="breadcrumb-item"><a href="'.correct_goto_url(G5_ADMIN_URL).'">Home</a></li><li class="breadcrumb-item active" aria-current="page">총판관리</li><li class="breadcrumb-item active" aria-current="page">'.$g5_title.'</li>';
?>
    <style>
        /* 모달 배경 스타일 */
        .modal2 {
...
<div class="admin-member-list">
    <div class="adm-headline">
        <h3>총판 리스트</h3>
        <?php if (!$wmode) { ?>
        <a href="<?php echo G5_ADMIN_URL; ?>/?dir=member&pid=member_form" class="btn-e btn-e-md btn-e-crimson adm-headline-btn m-l-5"><i class="las la-plus m-r-7"></i><span>총판 추가</span></a>
        <?php } ?>
    ...
    <div class="m-b-5">
        <div class="float-start f-s-13r">
            <a href="<?php echo G5_ADMIN_URL; ?>/?dir=<?php echo $dir; ?>&amp;pid=<?php echo $pid; ?>">[전체목록]</a><span class="m-l-10 m-r-10 text-light-gray">|</span>총 총판수 <?php echo number_format($total_count); ?>명<?php if (!$wmode) { ?> 중, <a href="<?php echo G5_ADMIN_URL; ?>/?dir=member&amp;pid=member_list&amp;sst=mb_intercept_date&amp;sod=desc&amp;sfl=<?php echo $sfl; ?>&amp;stx=<?php echo $stx; ?>"><u>차단 <?php echo number_format($intercept_count); ?></u></a>명, <a href="<?php echo G5_ADMIN_URL; ?>/?dir=member&amp;pid=member_list&amp;sst=mb_leave_date&amp;sod=desc&amp;sfl=<?php echo $sfl; ?>&amp;stx=<?php echo $stx; ?>"><u>탈퇴 <?php echo number_format($leave_count); ?></u></a>명<?php } ?>
        </div>
        <div class="float-end xs-float-start">
            <label for="sort_list" class="select width-200px">
                <select name="sort_list" id="sort_list" onchange="sorting_list(this.form, this.value);">
                    ...
                    <option value="mb_id|asc" <?php if ($sst=='mb_id' && $sod=='asc') { ?>selected<?php } ?>>총판아이디 정방향 (↓)</option>
                    <option value="mb_id|desc" <?php if ($sst=='mb_id' && $sod=='desc') { ?>selected<?php } ?>>총판아이디 역방향 (↑)</option>
            ...
            <table class="table table-bordered">
                <thead>
                    <tr>
                        ...
                        <th><?php echo $levelset['gnu_name']; ?></th>
                        <th>메일인증</th>
                        <th>메일수신</th>
                        <!--<th>SMS수신</th>-->
                        <th>접근차단</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-center">
                            <?php if ($is_admin!='group') { ?>
								<a href="<?php echo G5_ADMIN_URL; ?>/?dir=member&amp;pid=member_form&amp;mb_id=<?php echo $list[$i]['mb_id']; ?>&amp;w=u<?php if ($qstr) { ?>&amp;<?php echo $qstr; ?><?php } ?>"><u>수정</u></a>
							<?php } ?>
							<?php if ($config['cf_admin'] != $list[$i]['mb_id']) { ?>
								<a href="<?php echo G5_ADMIN_URL; ?>/?dir=board&amp;pid=boardgroupmember_form&amp;mb_id=<?php echo $list[$i]['mb_id']; ?>" class="m-l-10"><u>그룹</u></a>
							<?php } ?>
							</br><a class="btn-e btn-e-xs btn-e-red" href="javascript:openModal('<?php echo $list[$i]['mb_id']; ?>','<?php echo $list[$i]['mb_name']; ?>','<?php echo $list[$i]['mb_level2']; ?>');" class="m-l-10">등급변경/취소</a>
                        </td>
                        <?php } else { ?>
                        <td class="text-center">
                        ...
                        <td><strong><?php echo $list[$i]['mb_id']; ?></strong>
                            <!--<a <?php if (!(G5_IS_MOBILE || $wmode)) { ?>href="<?php echo G5_ADMIN_URL; ?>/?dir=member&pid=member_form&mb_id=<?php echo $list[$i]['mb_id']; ?>&w=u&amp;wmode=1" onclick="eb_modal(this.href); return false;"<?php } else { ?>href="javascript:void(0);"<?php } ?> ><i class="fas fa-external-link-alt text-light-gray m-r-7 hidden-xs"></i></a>
							</br><a class="btn-e btn-e-xs btn-e-blue" href="javascript:;">code:<?php echo $list[$i]['mb_recomm_code']; ?></a>-->
                        </td>
                        <td>
                        ...
                        <td class="text-center">
                            <a href="<?php echo G5_ADMIN_URL; ?>/?dir=member&pid=member_form&mb_id=<?php echo $list[$i]['mb_id'];; ?>&w=u&amp;wmode=1" onclick="eb_modal(this.href); return false;"><?php echo get_text($list[$i]['mb_dlevel']); ?>(<?php echo ($list[$i]['mb_point_rate']); ?>%)</a>
                        </td>
                        <?php if (!$wmode) { ?>
                        <!--<td>
    ...
    <div class="m-t-20">
        <div class="cont-text-bg">
            <p class="bg-info"><i class="fas fa-info-circle"></i> 총판자료 삭제 시 다른 총판이 기존 총판아이디를 사용하지 못하도록 총판아이디, 이름, 닉네임은 삭제하지 않고 영구 보관합니다.</p>
        </div>
    </div>
    <?php } ?>
</div>
...
<div class="modal fade admin-iframe-modal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title f-w-700">총판 정보 수정</h5>
	...
	<input type="hidden" name="mmb_id" id="mmb_id" value="">
	<input type="hidden" name="mmb_name" id="mmb_name" value="">
        <div class="modal-content">
            <h5 id='mtitle'>총판등급을 선택하세요</h5>
            <select id="d_mb_level">
                <option value="0">일반회원(총판취소)</option>
            ...
            <br>
            <button type="button" onclick="confirmSelection()">변경처리</button>
            <button type="button" onclick="closeModal2()">취소/닫기</button>
        </div>
    </div>

<script>
        // 모달 열기 함수
        function openModal(mb_id,mb_name,mb_level) {
			$("#mmb_id").val(mb_id);
			$("#mmb_name").val(mb_name);
			$("#d_mb_level").val(mb_level);
			$("#mtitle").html(mb_name+"("+mb_id+")님 총판등급을 선택하세요");
        ...
        function confirmSelection() {
            var selectedOption = $("#d_mb_level").val();
            var mb_id = $("#mmb_id").val();
            var mb_name = $("#mmb_name").val();
			var mlevel  = 0;
			
			if(selectedOption == 0) mlevel = "일반회원"
			else mlevel = "총판 "+selectedOption+"등급"; 

	if(confirm(mb_name+"("+mb_id+")님을 "+ mlevel +"으로 변경하시겠습니까?")) {
		$.ajax({
			type: "POST",
			url: g5_bbs_url+"/ajax.level_down.php",
			data: {
				"mb_id": mb_id,
				"mb_level": selectedOption
			},
			cache: false,
			async: false,
...
function levelDown(mb_id,mb_name) {
	if(confirm(mb_name+"("+mb_id+")님을 총판등급을 취소하고 일반회원 등급으로 변경하시겠습니까?")) {
		$.ajax({
			type: "POST",
			url: g5_bbs_url+"/ajax.level_down.php",
			data: {
